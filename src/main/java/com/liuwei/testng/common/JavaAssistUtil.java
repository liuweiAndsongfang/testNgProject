...
public class JavaAssistUtil {
    ...
    public static String[] getMethodParamNames(Class<?> clazz, String methodName, Class<?>[] paramTypes){
        try {
            ...
            CtMethod ctMethod = getMethodParamNames(ctClass.getDeclaredMethods(),methodName,paramTypes);
            if(ctMethod != null){
                return getMethodParamNames(ctMethod);
            }else {
                CtMethod cm;
                if(ctClass != null && ctClass.getSuperclass() != null){
                    cm = getMethodParamNames(ctClass.getSuperclass().getDeclaredMethods(), methodName,paramTypes);
                    if(cm != null){
                        return getMethodParamNames(cm);
                    }
                }
                cm = ctClass.getDeclaredMethod(methodName);
                if(cm != null) {
                    return getMethodParamNames(cm);
                }else {
                    CtMethod cm2 = getMethodParamNames(ctClass.getDeclaredMethods(), methodName, paramTypes);
                    return getMethodParamNames(cm2);
                }
            }
        }catch (Exception var7){
            throw new TestException("can not find method [name=" + methodName + "class= " + clazz + "]", var7);
        }
    }
    ...
    public static String[] getMethodParamNames(CtMethod ctMethod){
        ...
        try{
            ...
            throw new TestException("can not get method param types [method=" + ctMethod.getName() + "]", var8);
        }
    }
